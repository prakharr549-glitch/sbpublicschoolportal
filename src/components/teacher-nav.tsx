...
const navItems = [
  ...
  { href: "/teacher/classes", label: "My Classes", icon: Book },
  { href: "/teacher/students", label: "My Students", icon: Users },
  { href: "/teacher/grades", label: "Grades", icon: Award },
];

export function TeacherNav({ className }: React.HTMLAttributes<HTMLElement>) {
  const pathname = usePathname();

  return (
    <nav className={cn("flex flex-col gap-4", className)}>
      <Link href="/teacher-dashboard" className="flex items-center gap-2 px-4 text-lg font-bold text-primary">
