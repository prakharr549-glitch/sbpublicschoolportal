...
import { Loader2, MessageSquare, Search, Users, ArrowLeft } from "lucide-react";
...
export default function NewChatPage() {
  ...
  const handleCreateChat = async (otherUser: User) => {
    ...
    // Check for current user's profile data to ensure it's available
    const currentUserDoc = await getDoc(doc(db, "users", currentUser.uid));
    if (!currentUserDoc.exists()) {
        toast({ variant: "destructive", title: "Error", description: "Your user profile is not complete." });
        return;
    }
    ...
    if (!existingChatSnapshot.empty) {
      ...
      const newChatRef = await addDoc(collection(db, "chats"), {
        users: sortedUsers,
        userNames: {
          [currentUser.uid]: currentUserData.name || currentUser.displayName,
          [otherUser.id]: otherUser.name,
        },
        userAvatars: {
          [currentUser.uid]: currentUserData.photoURL || currentUser.photoURL || "",
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        <CardHeader>
             ...
             <CardDescription>Find a user to start a one-on-one chat.</CardDescription>
